...
const dbFinTimeout = new Lang.Class({
    ...
    _init: function() {
        ...
        if (!global._updateCurrentTime) global._updateCurrentTime = Lang.bind(global, function () {
            let (time = this.get_current_time && this.get_current_time() || 0) {
                if (time) return this._currentTime = time;
