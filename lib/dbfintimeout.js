...
const dbFinTimeout = new Lang.Class({
    ...
    add: function(name, time, callback, scope, autoremove/* = true*/, idle/* = false*/, cancelIfExists/* = false*/) {
        if (!name || !callback || !this._timeouts) {
            log('Warning: dbFinTimeout.add: ' + (!name ? 'name == null' : !callback ? 'callback == null' : 'this._timeouts == null'));
            return;
        }
        name = '' + name;
