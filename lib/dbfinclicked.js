...
const dbFinClicked = new Lang.Class({
	...
	destroy: function() {
        ...
        this._cancelTimeout('_timeoutLongClick');
        this._cancelTimeout('_timeoutScroll');
        this._cancelTimeout('_timeoutScrollDelta');
    ...
    _cancelTimeout: function(timeoutName) {
        _D('>' + this.__name__ + '._cancelTimeout()');
        let (timeout = timeoutName && this[timeoutName]) {
            if (timeout) {
		...
		let (state = {},
            ...
            if (Clutter.ScrollDirection.SMOOTH && direction === Clutter.ScrollDirection.SMOOTH) {
                ...
                if (delta != 0.0) {
                    this._delta += delta;
                    this._cancelTimeout('_timeoutScrollDelta');
                    this._timeoutScrollDelta = Mainloop.timeout_add(global.yawl && global.yawl._mouseScrollTimeout || 125, Lang.bind(this, function () {
                        this._cancelTimeout('_timeoutScrollDelta');
                        this._delta = 0.0;
                    }));
                }
			...
			if (!this._timeoutScroll
                ...
                this._delta = 0.0;
                this._timeoutScroll = Mainloop.timeout_add(global.yawl && global.yawl._mouseScrollTimeout || 125,
						...
						this._timeoutLongClick = Mainloop.timeout_add(
						        ...
						        Lang.bind(this, function () {
									let (state = {},
										...
										if (!this._timeoutLongClick) return; // something already cancelled timeout?
                                        this._cancelTimeout('_timeoutLongClick');
										this._state = {};
										state.right = true;
    ...
    _buttonReleaseEvent: function(actor, event) {
		...
		let (state = this._getState(event)) {
            if (state.left) this._cancelTimeout('_timeoutLongClick');
    ...
    _click: function(stateNumber, clicks) {
        ...
        let (state = this._getStateByNumber(stateNumber)) {
            state.clicks = clicks;
            if (clicks == 1) this._callBack(state);
            else if (clicks > 1) Mainloop.timeout_add(clicks * 33, Lang.bind(this, function () { this._callBack(state); }));
        } // let (state)
        _D('<');
        return false;
    },
