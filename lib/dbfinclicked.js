	...
	_getStateScroll: function(event) {
		...
		let (state = {},
            ...
            if (Clutter.ScrollDirection.SMOOTH && direction === Clutter.ScrollDirection.SMOOTH) {
                ...
                if (delta != 0.0) {
                    this._delta += delta;
                    if (this._timeout) this._timeout.add('scroll-delta', global.yawl && global.yawl._mouseScrollTimeout || 125, function () {
                        this._delta = 0.0;
                    }, this, true);
                }
            }
			...
			if ((!this._timeout || !this._timeout.has('scroll'))
                ...
                this._delta = 0.0;
                if (this._timeout) this._timeout.add('scroll', global.yawl && global.yawl._mouseScrollTimeout || 125, null, null, true);
			}
            _D('<');
            return state;
		} // let (state, direction)
            ...
            if (state.left || state.right || state.middle) {
				...
				if (!this._release) {
					...
					if (state.left && this._longClick) {
                        if (this._timeout) this._timeout.add('long-click', Math.max(1000, global.yawl._mouseClicksTimeThreshold * 2), function () {
                            let (state = {},
			...
			if (stateNumber) {
                if (!this._timeout || !this._timeout.remove(stateNumber)) { // first click
                    ...
                    if (this._double) { // if double clicks
                        this._timeout.add(stateNumber, global.yawl._mouseClicksTimeThreshold, null, null, true);
                    }
                }
                else { // second click
