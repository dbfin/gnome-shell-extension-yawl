...
const dbFinSlicerActor = new Lang.Class({
	...
	hoverEnter: function(actor) {
		...
		if (this.hoverAnimation && !this.hidden && !this.hiding) {
			let (state = {},
				...
				if (this.hoverFit && this.container) {
                    dbFinAnimation.animateToState(this.container, { min_width: this.getNaturalWidth() }, null, null, time, this.hoverAnimationEffect, true);
                }
			}
		}
        _D('<');
	...
	hoverLeave: function(actor) {
		...
		let (index = actor && this.hovered ? this.hovered.indexOf(actor) : -1) {
            if (index != -1) this.hovered.splice(index, 1);
        ...
        let (state = {},
            ...
            if (this.container) {
                dbFinAnimation.animateToState(this.container, { min_width: 0 }, null, null, time, this.hoverAnimationEffect, true);
            }
        }
        _D('<');
	},
