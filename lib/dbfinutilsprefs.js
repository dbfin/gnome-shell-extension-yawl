 ...
 *
 * 2) functions
 *  dbFinSettingsWelcome    create welcome page
 *
 *  dbFinSettingsREI    create reset/export/import page
 *
 */
...
/* function dbFinSettingsWelcome: create welcome page
 */
function dbFinSettingsWelcome(builder, name, title) {
    let (widgets = null) {
        builder.addPage(_("Welcome"));
			widgets = builder.addWidget(Gtk.Image.new_from_file(Me.path + '/images/' + name + '.png'), 0, 0, 3, 7);
			if (widgets && widgets.length) {
			...
			let (s = '') {
				...
				else builder.addLabel('<span size="small"> </span>', null, true);
			}
...
/* function dbFinSettingsREI    create reset/export/import page
 */
function dbFinSettingsREI(builder, name) {
    let (widgets = null) {
